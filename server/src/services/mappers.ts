...
export async function imageUrl(path: string | null | undefined, kind: ImageKind): Promise<string | null> {
if (!path) return null;
...
? d.credits.cast.slice(0, 8).map((c: any) => ({ name: c.name, role: c.character }))
: [],
