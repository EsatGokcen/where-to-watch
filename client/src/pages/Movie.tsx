...
export default function Movie() {
  ...
  const providersQ = useQuery({
    queryKey: ["providers", id, region],
    queryFn: () => apiProviders(id, region),
      ...
      <div className="grid md:grid-cols-[240px,1fr] gap-6">
        <div>
          {d.posterUrl ? (
            ...
            <div className="w-full aspect-[2/3] rounded-2xl border border-slate-800 grid place-items-center text-slate-500">
              No Image
            </div>
          )}
        </div>
        <div>
          <h1 className="text-2xl font-semibold">
            {d.title}{" "}
            {d.year ? <span className="text-slate-400">({d.year})</span> : null}
          </h1>
          {d.runtime ? (
            <div className="text-slate-400 text-sm mt-1">{d.runtime} min</div>
          ) : null}
          {d.genres?.length ? (
